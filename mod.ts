...
export async function onLoad(ctx: PluginContext): Promise<void> {
  ctx.logger.info(`[cortex-plugin-context7] Loaded`);
...
const context7_get_docs: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      const maxLength = typeof config.maxDocLength === 'string'
        ? parseInt(config.maxDocLength, 10) || 5000
        : 5000;
...
const context7_search: Tool = {
  definition: {
    ...
    params: [
      { name: 'query', type: 'string', description: 'Search query', required: true },
      {
        name: 'libraries',
        type: 'string',
        description: 'Comma-separated library names to search',
        required: false,
      },
      {
        name: 'max_results',
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      const libraries = typeof args.libraries === 'string'
        ? args.libraries.split(',').map((l) => l.trim().toLowerCase()).filter(Boolean)
        : null;
        ...
        for (const lib of libraries.slice(0, max_results)) {
          ...
          if (info) {
            results.push({
              ...
              snippet:
                `Documentation search results for "${query}" in ${info.name}. Connect Context7 for full results.`,
            });
          }
        }
      } else {
...
const context7_list_libraries: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      return {
        toolName: 'context7_list_libraries',
        success: true,
        output,
        durationMs: Date.now() - start,
      };
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!libraries || typeof libraries !== 'string') {
        return {
          toolName: 'context7_inject',
          success: false,
          output: '',
          error: 'libraries must be a non-empty string',
          durationMs: Date.now() - start,
        };
      }
      const topic = typeof args.topic === 'string' ? args.topic : '';
